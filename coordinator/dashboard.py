...
from shared.models import TaskMode, FileAttachment
...
async def api_chat_stream(request: Request, chat_request: StreamChatRequest):
    """
    Streaming chat endpoint using Server-Sent Events (SSE).

    Returns a stream of chunks as they're generated by the inference node.
    Rate limiting rules are the same as the regular chat endpoint.
    ...
    # Validate files for public chat (stricter limits)
    if chat_request.files:
        if len(chat_request.files) > 3:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Maximum 3 files allowed for public chat"
            )
        total_size = sum(f.size_bytes for f in chat_request.files)
        if total_size > 20 * 1024 * 1024:  # 20MB for public chat
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Total file size exceeds 20MB limit for public chat"
            )
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        # Use a longer timeout for the first chunk (model loading)
                        # and shorter for subsequent chunks
                        timeout = 120 if stream.chunks_received == 0 else 30
